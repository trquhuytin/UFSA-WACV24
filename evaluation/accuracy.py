...
class Accuracy(object):
    ...
    def mof(self, with_segments=False, old_gt2label=None, optimization='max'):
        ...
        print('# gt_labels: %d   # pr_labels: %d' %
                           (len(np.unique(self._gt_labels_subset)),
                            len(np.unique(self._predicted_labels))))
        print('Correspondences: segmentation to gt : '
    ...
    def mof_classes(self):
        ...
        for key, val in self._classes_MoF.items():
            ...
            print('label %d: %f  %d / %d' % (key, true_frames / all_frames,
        ...
        print('average class mof: %f' % average_class_mof)
        self._return['mof'] = [self._frames_true_pr, self._frames_overall]
        self._return['mof_bg'] = [total_true, total]
        if self.opt.bg:
            print('mof with bg: %f' % (total_true / total))
        print("mof : %f"%(self._frames_true_pr/self._frames_overall))
    def iou_classes(self):
        average_class_iou = 0
        excluded_iou = 0
        for key, val in self._classes_IoU.items():
            true_frames, union = val
            print('label %d: %f  %d / %d' % (key, true_frames / union,
                                                    true_frames, union))
        ...
        print('average IoU: %f' % average_iou_without_exc)
        self._return['iou'] = [average_class_iou,
                               len(self._classes_IoU) - len(self.exclude)]
        self._return['iou_bg'] = [average_class_iou + excluded_iou,
                                  len(self._classes_IoU) - len(self.exclude)]
        if self.exclude:
            print('average IoU with bg: %f' % average_iou_with_exc)


    def mof_val(self):
        print('frames true: %d\tframes overall : %d' %
