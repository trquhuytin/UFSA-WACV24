...
class FrameWiseLoss(nn.Module):
    ...
    def __call__(self, predictions,q, batch_target, mask, epoch):      
        # initialize losses
        ...
        if self.args.do_framewise_loss_gauss : 
            sum= torch.sum(q * torch.log(predictions), dim=0)
            
            framewise_loss = framewise_loss-torch.mean(sum)
    ...
    def update_metrics(self, predictions, batch_target, g_pred, g_gt, mask, loss,framewise_loss, framewise_loss_g, framewise_loss_random_singleframe):
        ...
        if self.args.do_framewise_loss_gauss:
            self.acc.update(0, n= len(predictions))
...
class SegmentLossAction(nn.Module):
    ...
    def __call__(self, pred_action, target_action,vid_len, q_segment, epoch):
...
class AttentionLoss(nn.Module):
    ...
    def __call__(self, pred_attn, target_attn, batch_target, q):  
        ...
        if self.args.do_crossattention_action_loss_nll or self.args.do_framewise_alignment_loss:
            ...
            for i in range(len(pred_attn)):
                ...
                for batchnum in range(pred_attn[i].shape[0]):
                    ...
                    #valid_pred_attn_log = F.softmax(valid_pred_attn_lln/self.args.temperature)
                    valid_pred_attn_log = self.LogSoftmax(valid_pred_attn_lln/self.args.temperature)
                    #attn_loss_tmp = attn_loss_tmp + (self.NLLLoss(valid_pred_attn_log, q.argmax(0).view(-1).long()) * float(1 / pred_attn[i].shape[0]))
                    ...
                    #proto_probs = torch.clamp(valid_pred_attn_log, min= 1e-30, max=1)
                    #proto_loss = torch.mean(torch.sum(valid_target_attn_lln*torch.log(proto_probs), dim = 1))
                    #attn_loss_tmp = attn_loss_tmp + proto_loss

            attn_loss_nll = attn_loss_tmp / (len(pred_attn) + 0.0)
            #print("loss py : ", attn_loss_nll)
        loss = attn_loss_nll
