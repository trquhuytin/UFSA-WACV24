...
def write_metrics_mof_f1( mof , f1, epoch, filename):
    content = f'MOF {mof:.4f} F1 {f1:.4f}'
    write_log(content, epoch, filename)



def write_return_stats(return_stats, epoch, filename):
    keys = ["mof", "precision", "recall", "mean_f1"]
    content= ""
    for key in keys:
        v1, v2=return_stats[key]
        content += f'{key} {v1:.5f} {v2:.5f} '
