...
class TrainDataGenerator(data.Dataset):
    def __init__(self,
                 data_root=None,
                 split=None,
                 dataset=None,
        ...
        if mode.lower() == 'train':
            ...
            # file_ptr.close()
            self.list_of_examples = os.listdir(data_root+"/"+dataset+'/'+'features')
            print('we have in total {} training data'.format(len(self.list_of_examples)))
            sample_rate = 1 #args.sample_rate
        elif mode.lower() == 'val':
        ...
        for indv, valv in enumerate(self.list_of_examples):
            self.data[indv] = dict()
            self.data[indv]['name'] = gt_path + valv.split('.')[0]
            if feature_type == ".npy":
            ...
            file_ptr = open(gt_path + valv.split('.')[0]+'.txt', 'r')
            content = file_ptr.read().split('\n')[:-1]
